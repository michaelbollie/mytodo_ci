...
const quoteFormSchema = z.object({
  ...
  status: z.enum(["draft", "sent", "accepted", "rejected"], { required_error: "Status is required." }),
...
interface QuoteFormProps {
  initialData?: QuoteFormValues & { id?: string; quote_number?: string }
}
...
export function QuoteForm({ initialData }: QuoteFormProps) {
  ...
  const onSubmit = async (values: QuoteFormValues) => {
    ...
    try {
      ...
      const url = initialData?.id ? `/api/quotes/${initialData.id}` : "/api/quotes"
      ...
      if (res.ok) {
        ...
        router.refresh() // Revalidate data on the quotes list page
      } else {
        setError(data.message || `Failed to ${initialData?.id ? "update" : "create"} quote.`)
      }
    } catch (err: any) {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Select
          onValueChange={(value) => form.setValue("status", value as QuoteFormValues["status"])}
      ...
      <Button type="submit" className="w-full" disabled={isSubmitting}>
        {isSubmitting ? "Saving..." : initialData?.id ? "Update Quote" : "Create Quote"}
      </Button>
    </form>
  )
}
